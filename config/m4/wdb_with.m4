# -*- autoconf -*-
#
# $Id: wdb_with.m4 181 2012-05-24 14:11:34Z aleksandarb $
#
# SYNOPSIS
#
#   WDB_WITH_LIBRARY(variable-prefix, option-name, pretty-name, default)
#   WDB_WITH(variable-prefix, option-name, pretty-name, default)
#   WDB_IF_WITH(option-name, action-if-with, action-if-without)
#   WDB_IF_WITH_ANY(option-names, action-if-with, action-if-without)
#   WDB_IF_WITH_ALL(option-names, action-if-with, action-if-without)
#   WDB_WITH_CONDITIONAL(variable-prefix, option-name)
#
# DESCRIPTION
#
#   The WDB_WITH_LIBRARY macro creates three command-line options
...
#
#   The WDB_IF_WITH_ALL macro executes <action-if-with> if the
#   with_<option> shell variable is "yes" for all of the specified
#   options, and <action-if-without> otherwise.  Either or both
#   actions can be empty.
#
#   The WDB_WITH_CONDITIONAL macro defines an automake conditional
...
#   Better help strings.
#
#   Better handling of default case (see wdb_enable.m4)
#
# AUTHORS
#
...
# $3 = action-if-without
#
AC_DEFUN([WDB_IF_WITH_ANY], [
    wdb_if_with_any_result=no
    m4_foreach_w(COMPONENT, [$1], [
	AS_IF([test x"$with_[]COMPONENT" = x"yes"],
	    [wdb_if_with_any_result=yes; break])
    ])
    AS_IF([test x"$wdb_if_with_any_result" = x"yes"], [:;$2], [:;$3])
    unset wdb_if_with_any_result
])
...
# $3 = action-if-without
#
AC_DEFUN([WDB_IF_WITH_ALL], [
    wdb_if_with_all_result=yes
    m4_foreach_w(COMPONENT, [$1], [
	AS_IF([test x"$with_[]COMPONENT" != x"yes"],
	    [wdb_if_with_all_result=no; break])
    ])
    AS_IF([test x"$wdb_if_with_all_result" = x"yes"], [:;$2;], [:;$3])
    unset wdb_if_with_all_result
])

#
# WDB_WITH_CONDITIONAL
#
# $1 = variable-prefix
# $2 = option-name
#
AC_DEFUN([WDB_WITH_CONDITIONAL], [
